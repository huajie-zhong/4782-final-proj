...
\titleformat{\section}{\normalfont\large\bfseries\color{cornellred}}{\thesection.}{0.5em}{}[\vspace{-0.5ex}\color{cornellred}\titlerule]
\titlespacing*{\section}{0pt}{8pt}{4pt}

\title{\vspace{-2em}\textbf{Reproducing MEDUSA:}\\[2pt]
  \large Speculative Decoding with Multiple Drafting Heads on Vicuna-7B}
\author{Huajie Zhong (hz642) \qquad Frank Dai (sd924)\\[4pt]
...
LLM inference is bottlenecked by sequential token generation: each forward pass yields only one token, and at batch size one the GPU is memory-bandwidth-bound rather than compute-bound~\cite{pope2022efficiently}. Speculative decoding~\cite{leviathan2023fast,chen2023accelerating} addresses this by drafting multiple tokens cheaply and verifying them in parallel with the target model. \textbf{MEDUSA}~\cite{cai2024medusa} replaces the separate draft model with $K$ extra decoding heads attached to a frozen backbone, each predicting a token several steps ahead. A tree-attention verification pass checks all candidate continuations in one forward call, and an acceptance criterion preserves the model's output distribution. We reproduce MEDUSA-1 on Vicuna-7B~\cite{vicuna2023}, targeting the 2.18$\times$ speedup reported in \S3.1 and the technique ablation in Table 3 of the paper.
...
\textbf{Inference loop} (\emph{propose $\to$ build tree $\to$ verify $\to$ accept $\to$ KV surgery}). We use a static 64-node BFS-pruned tree with top-$s_k$ widths $s=[10,3,2,2]$; the BFS order preserves parent-before-child under truncation. The tree attention mask lets each node attend only to the prefix tokens and its own ancestors (not siblings or cousins), so all 64 candidate continuations are verified in a single forward pass; after acceptance we \texttt{index\_select} the KV entries along the accepted root-to-leaf path. We implement both \emph{greedy} acceptance ($\arg\max{=}$child $\Rightarrow$ bitwise-identical to the autoregressive baseline) and \emph{typical} acceptance (\S2.3.1, after typical sampling~\cite{meister2023typical}): $p_{\text{orig}}(x) > \min(\varepsilon,\,\delta e^{-H(p)})$ with $\varepsilon{=}\delta{=}0.09$, distributionally equivalent to the original.

\textbf{Ablation modes / deviations.} We compare \emph{none} (no tree), \emph{naive} (full 220-node Cartesian tree with a dense causal mask), and \emph{optimized} (BFS-pruned 64-node sparse ancestor-only mask + typical acceptance). We use $K{=}4$ rather than the paper's $K{=}5$ (training-memory), train on raw ShareGPT rather than Vicuna-regenerated responses, and smoke-test on TinyLlama-1.1B~\cite{zhang2024tinyllama} before scaling.

\begin{figure}[H]
\centering
\begin{minipage}[c]{0.42\linewidth}\centering
\begin{tikzpicture}[font=\small,
    headnode/.style={draw=cornellred, rounded corners=2pt, fill=cornellred!10, minimum width=24mm, minimum height=5.5mm, drop shadow, font=\bfseries},
    hnode/.style={draw=gray, rounded corners=2pt, fill=gray!15, minimum width=24mm, minimum height=5.5mm, drop shadow, font=\bfseries}]
  \node[hnode] (h) at (0,0) {Hidden states $h$};
  \node[headnode] (lm) at (0,-1.0) {LM head $\to$ $t{+}1$};
  \node[headnode] (h0) at (0,-1.7) {Head 0 $\to$ $t{+}2$};
  \node[headnode] (h1) at (0,-2.4) {Head 1 $\to$ $t{+}3$};
  \node[headnode] (h2) at (0,-3.1) {Head 2 $\to$ $t{+}4$};
  \node[headnode] (h3) at (0,-3.8) {Head 3 $\to$ $t{+}5$};
  \draw[-{Latex[length=1.4mm]}, thick, cornellred] (h.south) -- (lm.north);
  \draw[thick, cornellred] (h.west) -- ++(-0.6,0) coordinate (trunk);
  \draw[-{Latex[length=1.4mm]}, thick, cornellred, rounded corners=2pt] (trunk) |- (h0.west);
  \draw[-{Latex[length=1.4mm]}, thick, cornellred, rounded corners=2pt] (trunk) |- (h1.west);
  \draw[-{Latex[length=1.4mm]}, thick, cornellred, rounded corners=2pt] (trunk) |- (h2.west);
  \draw[-{Latex[length=1.4mm]}, thick, cornellred, rounded corners=2pt] (trunk) |- (h3.west);
\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}[c]{0.55\linewidth}\centering
\begin{tikzpicture}[font=\small,
    cnode/.style={draw=darkgray, circle, minimum size=7mm, fill=gray!20, drop shadow},
    rnode/.style={draw=cornellred!80!black, circle, minimum size=7mm, fill=cornellred, text=white, drop shadow, font=\bfseries},
    anode/.style={draw=cornellred!80!black, circle, minimum size=7mm, fill=cornellred!40, drop shadow},
    cnode_highlight/.style={draw=cornellred!80!black, circle, minimum size=7mm, fill=cornellred!70, text=white, drop shadow, font=\bfseries}]
  \node[cnode] (p1) at (-1.8,1.0) {$P_1$};
  \node[cnode] (p2) at (-0.8,1.0) {$P_2$};
  \node[rnode] (r) at (1.0,1.0) {root};
  \node[anode] (a) at (0.0,-0.5) {A};
  \node[cnode] (b) at (2.0,-0.5) {B};
  \node[cnode_highlight] (c) at (-1.0,-2.0) {C};
  \node[cnode] (d) at (1.0,-2.0) {D};
  ...
  \draw[-{Latex[length=1.5mm]}, thick, gray] (p2) -- (r);
  \node[right=2mm of b, text width=38mm, align=left, font=\small]
    {\textbf{Attention Mask for C:}\\ Can see \textcolor{gray}{prefix ($P_1,P_2$)},\\ \textcolor{cornellred}{ancestors (root, A)}, and self.\\ \emph{Cannot} see siblings (B, D).};

  \begin{scope}[on background layer]
    \draw[cornellred!15, line width=10mm, line cap=round, line join=round] (r.center) -- (a.center) -- (c.center);
    \draw[gray!15, line width=10mm, line cap=round, line join=round] (p1.center) -- (p2.center);
  \end{scope}
\end{tikzpicture}
\end{minipage}
...
Acceptance length (eff.\ tokens/step)            & \textbf{3.47} (MED-2) & 2.87 \\
\bottomrule
\end{tabular}
...
\textbf{AI usage disclosure.} We used Anthropic Claude (Claude Code)~\cite{claudecode} as a coding assistant for boilerplate scaffolding (argparse, plotting, JSON I/O), debugging KV-cache shape mismatches, and iterating on this writeup's prose and \LaTeX{} formatting. All algorithmic decisions (tree topology, label-shift convention, acceptance criteria, ablation design) and all reported numbers were produced and verified by the authors; no AI-generated text was included without human review.
...
\bibitem{vicuna2023} W.-L.~Chiang et al. \emph{Vicuna: An Open-Source Chatbot Impressing GPT-4 with 90\%* ChatGPT Quality.} LMSYS, 2023.
\bibitem{zhang2024tinyllama} P.~Zhang, G.~Zeng, T.~Wang, W.~Lu. \emph{TinyLlama: An Open-Source Small Language Model.} arXiv:2401.02385, 2024.
...
\bibitem{chen2023accelerating} C.~Chen, S.~Borgeaud, G.~Irving, J.-B.~Lespiau, L.~Sifre, J.~Jumper. \emph{Accelerating Large Language Model Decoding with Speculative Sampling.} arXiv:2302.01318, 2023.
\bibitem{meister2023typical} C.~Meister, T.~Pimentel, G.~Wiher, R.~Cotterell. \emph{Locally Typical Sampling.} TACL, 2023.
\bibitem{pope2022efficiently} R.~Pope et al. \emph{Efficiently Scaling Transformer Inference.} arXiv:2211.05102, 2022.
\bibitem{claudecode} Anthropic. \emph{Claude Code.} \url{https://www.anthropic.com/claude-code}, 2025.
\end{thebibliography}
