...
export default function TreeSection() {
  ...
  return (
    <section id="section-3" className="min-h-screen flex flex-col justify-center px-8 py-20 bg-white border-t border-birch">
      <div className="max-w-5xl mx-auto w-full">
        ...
        >
          ...
          {/* Depth stats */}
          <motion.div variants={fadeUp} className="grid grid-cols-4 gap-3 text-center">
            {[1, 2, 3, 4].map(d => (
              <div key={d} className="bg-parchment border border-birch rounded p-3">
                <div className="text-xl font-bold font-mono" style={{ color: getDepthColor(d) }}>
                  {DEPTH_COUNTS[d - 1]}
                </div>
                <div className="text-xs text-ink-soft mt-0.5">depth {d} nodes</div>
                <div className="text-xs text-birch font-mono">top-{S_K[d - 1]}/parent</div>
              </div>
            ))}
          </motion.div>
